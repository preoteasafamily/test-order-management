...
const anafRouter = require("./routes/anaf");
...
app.get("/api/orders", (req, res) => {
  try {
    const rows = db
      .prepare("SELECT * FROM orders ORDER BY date DESC, createdAt DESC")
      .all();
...
app.post("/api/orders", (req, res) => {
  ...
  if (!order.id || !order.date || !order.clientId) {
    return res
      .status(400)
      .json({ error: "id, date, and clientId are required" });
  }
...
app.post("/api/clients", (req, res) => {
  ...
  if (client.agentId) {
    try {
      ...
      if (!agent) {
        ...
        return res
          .status(400)
          .json({ error: `Agent with ID '${client.agentId}' does not exist` });
      }
    } catch (err) {
      console.error("❌ Error validating agent:", err);
      return res
        .status(500)
        .json({ error: "Error validating agent: " + err.message });
    }
  }
  ...
  if (client.priceZone) {
    try {
      const zone = db
        ...
        .get(client.priceZone);
      if (!zone) {
        console.error(`❌ Price zone not found: ${client.priceZone}`);
        return res.status(400).json({
          error: `Price zone with ID '${client.priceZone}' does not exist`,
        });
      }
    } catch (err) {
      ...
      return res
        .status(500)
        .json({ error: "Error validating price zone: " + err.message });
    }
  }
  ...
  if (!["active", "inactive", "periodic"].includes(status)) {
    return res
      .status(400)
      .json({ error: "Status must be active, inactive, or periodic" });
  }

  if (status === "periodic") {
    if (!client.activeFrom || !client.activeTo) {
      return res.status(400).json({
        error: "activeFrom and activeTo are required for periodic status",
      });
    }
    if (client.activeFrom > client.activeTo) {
      return res
        .status(400)
        .json({ error: "activeFrom must be before or equal to activeTo" });
    }
  }
...
app.post("/api/clients/bulk-status", (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ Bulk status update: ${updatedCount} clients updated to ${status}`,
    );
...
// ============ AGENT LOCATIONS ENDPOINTS (FROM MAIN SERVER) ============

// Get all agent locations with names
app.get("/api/agents-locations", (req, res) => {
  try {
    const rows = db
      .prepare("SELECT * FROM agent_locations ORDER BY timestamp DESC")
      .all();
    res.json(rows || []);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
// Check if SSL certificates exist in certs folder
if (fs.existsSync("./certs/key.pem") && fs.existsSync("./certs/cert.pem")) {
  ...
  console.warn("⚠️  SSL certificates not found in ./certs/ folder.");
  console.warn(
    "Generate certificates with: openssl req -x509 -newkey rsa:4096 -keyout certs/key.pem -out certs/cert.pem -days 365 -nodes",
  );
